...
- **Date**: current date
- **Model**: the Claude model used (e.g., Claude Opus 4.6)
- **Sub-agents**: number of parallel fluxcd agents launched
...
1. **Overview cards** — one card per repo in a responsive grid:
   ...
   - Key metrics rendered as chip spans — bold count + muted kind label, flex-wrapped:
     ...
     ```
   - No `<br>` tags — chips reflow naturally via `flex-wrap`
   - No description here — descriptions go under each repo's detail section

2. **Comparison table** — columns: Check | repo1 | repo2 | ...
   - Rows: YAML Validation, Schema Validation (with resource count), Deprecated APIs, Secrets Management, Cosign Verification, Drift Detection, Multi-Tenancy
...
One `<h2>` section per repository. Each contains:
...
3. **Security Checks** — checklist using `check-pass`, `check-warn`, `check-fail` CSS classes:
   ...
   - ⚠ for warnings (amber)
   - ✗ for failures (red)
4. **Best Practices Checks** (if substantial) — same checklist format
...
```html
<footer>{Project} GitOps Audit — Generated by Flux CD agents — {YYYY-MM-DD}</footer>
```
